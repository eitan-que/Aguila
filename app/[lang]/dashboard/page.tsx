...
export default async function DashboardRoot({ params }: { params: Promise<{ lang: Lang }> }) {
  ...
  if ('error' in metricsRes && !('metrics' in metricsRes)) {
    return (
      <ErrorState
        title={dict.dashboard.error.title}
        description={dict.dashboard.error.description}
        retryHref={`/${lang}/dashboard`}
        retryLabel={dict.dashboard.error.retry}
      />
    );
  }
  const m: any = (metricsRes as any).metrics;
  const d = dict.dashboard;
  return (
    <PageShell>
      <PageHeader title={d.title} />
      <PageSections>
        <Section title={d.sections.overview}>
        ...
        <Section title={d.sections.engagement}>
        ...
        <Section title={d.sections.users}>
        ...
        <Section title={d.sections.extremes}>
          <MetricCard label={d.labels.mostVisited} value={m.restaurantExtremes.mostVisited?.name || '-'} hint={`${m.restaurantExtremes.mostVisited?.visits ?? 0} visits`} />
          <MetricCard label={d.labels.leastVisited} value={m.restaurantExtremes.leastVisited?.name || '-'} hint={`${m.restaurantExtremes.leastVisited?.visits ?? 0} visits`} />
